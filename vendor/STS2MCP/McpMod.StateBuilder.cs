...
public static partial class McpMod
{
    ...
    private static Dictionary<string, object?> BuildPlayerState(Player player)
    {
        ...
        // Permanent deck, not transient combat piles. Needed for strategic decisions.
        var permanentDeck = new List<Dictionary<string, object?>>();
        int deckIndex = 0;
        foreach (var card in player.Deck.Cards)
        {
            // Use the same resolved representation as hand/reward cards. card.Description is a
            // localization template and can expose unresolved {Damage:diff()} placeholders.
            var cardInfo = BuildCardInfo(card);
            cardInfo["deck_index"] = deckIndex++;
            permanentDeck.Add(cardInfo);
        }
        state["deck"] = permanentDeck;
    ...
    private static Dictionary<string, object?> BuildCardInfo(CardModel card, PileType pile = PileType.None)
    {
        var result = new Dictionary<string, object?>
        {
            ...
            ["enchantments"] = card.Enchantment == null
                ...
                : new List<Dictionary<string, object?>>
                {
                    new()
                    {
                        ...
                        ["name"] = SafeGetText(() => card.Enchantment.Title),
                        ["description"] = SafeGetText(() => card.Enchantment.DynamicDescription),
                        ["amount"] = card.Enchantment.Amount,
                        ["status"] = card.Enchantment.Status.ToString()
                    }
                },
            ["affliction"] = card.Affliction == null ? null : new Dictionary<string, object?>
            {
                ["id"] = card.Affliction.Id.Entry,
                ["name"] = SafeGetText(() => card.Affliction.Title),
                ["description"] = SafeGetText(() => card.Affliction.DynamicDescription),
